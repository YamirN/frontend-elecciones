...
const estudiantes = computed(() => estudianteStore.estudiantes);
...
const onPageChange = (event) => {
    estudianteStore.obtenerEstudiantes(event.page + 1); // PrimeVue empieza desde 0
};
...
// const deleteEstudianteDialog = ref(false);
...
<template>
    <div>
        <div class="card">
            <Toolbar class="mb-6">
                <template #start>
                    <Button label="Registrar Elector" icon="pi pi-plus-circle" severity="secondary" class="mr-2" @click="openNew" />

                    <div>
                        <Button label="Eliminar todos los electores" icon="pi pi-trash" severity="danger" class="mr-2" @click="showDeleteConfirmation" />
                    </div>
                </template>
            ...
            <DataTable :value="estudiantes" :filters="filters" :lazy="true" :paginator="true" :rows="pagination.per_page" :totalRecords="pagination.total" :first="(pagination.current_page - 1) * pagination.per_page" @page="onPageChange" dataKey="id">
                <template #header>
                ...
                <Column field="aula" header="Aula" style="width: 15%"></Column>
                <Column field="nivel" header="Nivel" sortable style="width: 15%"></Column>
                <Column field="fecha_nacimiento" header="Fecha Nacimiento" style="width: 20%"></Column>
        ...
        <Dialog v-model:visible="voterCreateDialog" :modal="true" :style="{ width: '30rem' }" :closable="false">
            <div class="flex flex-col gap-5 px-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Select v-model="form.seccion" :options="SeccionOptions" optionLabel="label" optionValue="value" placeholder="Elije la seccion" :invalid="!!errors.seccion" class="w-full md:w-56" />
                    ...
                    <div>
                        ...
                        <Select v-model="form.grado" :options="gradoOptions" optionLabel="label" optionValue="value" placeholder="Selecciona el grado" class="w-full md:w-56" />
            ...
            <template #footer>
                ...
                <Button label="Guardar" class="custom-cancel-button" text icon="pi pi-check" @click="handleSave" :disabled="!isValidSelection" />
            </template>
        </Dialog>
        ...
        <Dialog v-model:visible="deleteVoterDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
            <div class="flex items-center gap-4">
                ...
                <span class="text-red-500" v-if="selectedVoter"
                    >Estas seguro que quieres eliminar a <b>{{ selectedVoter.nombre }} {{ selectedVoter.apellido }}</b
                    >?</span
                >
            </div>
            <template #footer>
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600">
                            ...
                            >
                                ...
                                <input id="file-upload" name="file-upload" type="file" class="sr-only" @change="handleFileUpload" accept=".xlsx, .xls" />
                            </label>
    ...
    <Dialog v-model:visible="isLoading" modal :closable="false" :showHeader="false" :style="{ width: '300px' }">
        <div class="flex flex-column align-items-center p-5">
            ...
            <p class="text-center text-sm text-gray-600">Por favor, espere mientras se procesan los datos del archivo Excel. Este proceso puede tardar unos minutos dependiendo del tamaño del archivo.</p>
        </div>
    </Dialog>
