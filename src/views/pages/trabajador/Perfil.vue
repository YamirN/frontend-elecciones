...
const form = ref({
    ...
    descripcion: user.modelo?.descripcion || '',
    telefono: user.modelo?.telefono || '',
    password: '',
...
const validateForm = () => {
    ...
    // Teléfono: si lo ingresó, validar que tenga máximo 9 dígitos y sea numérico
    if (form.value.telefono) {
        const telefonoRegex = /^[0-9]{1,9}$/;
        if (!telefonoRegex.test(form.value.telefono)) {
            errors.value.telefono = 'El teléfono debe tener hasta 9 dígitos numéricos';
        }
    }
    ...
    if (form.value.password || form.value.password_confirmation) {
        ...
        if (!form.value.password_confirmation) {
            errors.value.password_confirmation = 'Debes confirmar la contraseña';
        }
...
const guardarCambios = async () => {
    ...
    loading.value = true;
    try {
        await trabajadorStore.actualizarPerfil(form.value);
...
<template>
    <div class="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
        <h2 class="text-2xl font-bold text-gray-800">Perfil del Trabajador</h2>
        ...
        <div>
            ...
            <input type="text" :value="user.modelo?.dni" class="w-full mt-1 input-disabled" disabled />
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700">Hora de inicio</label>
            <input :value="user.modelo?.horario_inicio" type="time" class="w-full mt-1 input-disabled" disabled />
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700">Hora de fin</label>
            <input :value="user.modelo?.hora_fin" type="time" class="w-full mt-1 input-disabled" disabled />
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input v-model="form.telefono" type="text" class="w-full mt-1 input" :class="{ 'border-red-500': errors.telefono }" />
                <p v-if="errors.telefono" class="text-red-600 text-sm mt-1">{{ errors.telefono }}</p>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700">Descripción</label>
                <textarea v-model="form.descripcion" rows="3" class="w-full mt-1 input"></textarea>
            </div>
