...
onMounted(async () => {
    // 1️⃣ Carga inicial
    await eventosStore.obtenerEventosVotar();
    await nextTick();
...
<template>
    <div class="p-6 bg-white rounded-lg shadow-lg">
        <!-- Logs del Sistema -->
        <DataTable
            :value="eventosStore.eventosvotar"
            :paginator="true"
            :rows="10"
            :filters="filters"
        ...
        >
            ...
            <template #header>
                <div class="flex flex-col gap-3 p-3 bg-gray-50 rounded-t-2xl">
                    ...
                    <div class="flex flex-wrap items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-700 flex items-center gap-2"><i class="pi pi-history text-blue-500"></i> Registro de votacion</h3>
            ...
            <Column header="Nombre" style="width: 25%">
                <template #body="slotProps">
                    <span class="px-2 py-1 text-sm font-semibold rounded-md"> {{ slotProps.data.usuario_nombre }} {{ slotProps.data.usuario_apellido }} </span>
                </template>
            </Column>
            ...
            <Column header="Acción" style="width: 15%">
                <template #body="slotProps">
                    <span class="text-gray-700">{{ slotProps.data.accion }}</span>
                </template>
            </Column>

            <!-- COLUMNA: Nivel -->
            <Column header="Nivel" style="width: 10%">
                <template #body="slotProps">
                    <span :class="getNivelBadge(slotProps.data.nivel)" class="px-2 py-1 text-sm font-semibold rounded-md">
            ...
            <Column header="Estado" style="width: 10%">
                <template #body="slotProps">
                    <span :class="['px-2 py-1 text-sm font-semibold rounded-md', slotProps.data.exitoso ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700']">
                        {{ slotProps.data.exitoso ? 'Exitoso' : 'Fallido' }}
                    </span>
                </template>
            </Column>
